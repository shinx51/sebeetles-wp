...
class pdfemb_SecureUploader {
	...
	public function __construct($pdfemb_secure=true) {
	...
	public function handle_downloads() { // Called in init
		...
		if ($filepath != '') {
			...
			$myrc4 = $direct_download
                ? new pdfembDirectRC4('')
                : new pdfembSimpleRC4($this->getSecretKey());
	...
	public function getSecurePath($pdfurl) {
		...
		if (preg_match($regex, set_url_scheme($pdfurl), $matches)) {
			...
			if (strpos($pdfurl, '..') === FALSE && strpos($pdfurl, '\\') === FALSE) {
	...
	protected function readfile_chunked( $file, $retbytes=true, $myrc4 ) {
	...
	public function getSecretKey() {
		$sk = get_site_transient("pdfemb-sk");
		
		if ($sk === FALSE) {
			$sk = md5(sprintf("pdfemb-%d-%d", rand(), time()));
			set_site_transient("pdfemb-sk", $sk, 60*60*12);
		}
